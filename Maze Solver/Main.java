...
class MazeSolver implements IMazeSolver {
    ...
    static int[][] find_path_BFS( LinkedListQueue Queue_cost , LinkedListQueue Queue_x , LinkedListQueue Queue_y , int cost ) {
        ...
        while ( Queue_cost.size() > 1 ) {

            current_cost = (int) Queue_cost.dequeue() ;

            if ( current_cost == step && current_cost < cost ) {


                path[step - 1][1] = (int) Queue_x.dequeue() ;
                path[step - 1][0] = (int) Queue_y.dequeue() ;
                step++ ;
    ...
    public class Main {
        ...
        static void print_maze_with_path(char[][] maze, int[][] path) {
            for (int[] step : path) {
                int x = step[0];
                int y = step[1];
        ...
        public static void main(String[] args) throws IOException {
            ...
            try {
                ...
                if ( path != null ) {
                    ...
                    print_path(path);
                    char[][] maze = readMazeFromFile(file);
        ...
        static char[][] readMazeFromFile(File file) throws IOException {
            System.out.println();
            System.out.println();
